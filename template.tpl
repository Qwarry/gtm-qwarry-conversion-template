___TERMS_OF_SERVICE___

By creating or modifying this file you agree to Google Tag Manager's Community
Template Gallery Developer Terms of Service available at
https://developers.google.com/tag-manager/gallery-tos (or such other URL as
Google may provide), as modified from time to time.


___INFO___

{
  "type": "TAG",
  "id": "cvt_temp_public_id",
  "version": 1,
  "securityGroups": [],
  "displayName": "Qwarry Conversion",
  "categories": [
    "CONVERSIONS",
    "ATTRIBUTION"
  ],
  "brand": {
    "id": "brand_dummy",
    "displayName": "",
    "thumbnail": "data:image/png;base64,\u003d\u003d"
  },
  "description": "Conversion measurement for Qwarry (can be used as a simple visit tag)",
  "containerContexts": [
    "WEB"
  ]
}


___TEMPLATE_PARAMETERS___

[
  {
    "type": "RADIO",
    "name": "tagType",
    "displayName": "How do you want to use this tag ?",
    "radioItems": [
      {
        "value": "visit",
        "displayValue": "As a visit tag",
        "help": "Use it if you want to treat a simple visit on the landing page"
      },
      {
        "value": "conversion",
        "displayValue": "As a conversion tag",
        "help": "Use it if you want to treat a conversion involving multiple pages"
      }
    ],
    "simpleValueType": true
  },
  {
    "type": "RADIO",
    "name": "postbackMethod",
    "displayName": "How the conversion notification to Qwarry is made ?",
    "radioItems": [
      {
        "value": "pixel",
        "displayValue": "Using a pixel",
        "help": "The tag will fire a pixel to notify Qwarry"
      },
      {
        "value": "iframe",
        "displayValue": "Using a hidden iframe",
        "help": "The tag will spawn a hidden iframe to notify Qwarry"
      }
    ],
    "simpleValueType": true,
    "valueValidators": [
      {
        "type": "NON_EMPTY"
      }
    ],
    "defaultValue": "pixel"
  },
  {
    "type": "TEXT",
    "name": "advertiserId",
    "displayName": "Advertiser ID",
    "simpleValueType": true,
    "valueValidators": [
      {
        "type": "NON_EMPTY"
      }
    ],
    "help": "Your Qwarry advertiser ID"
  },
  {
    "type": "TEXT",
    "name": "customValue",
    "displayName": "Custom value",
    "simpleValueType": true,
    "defaultValue": 1,
    "valueValidators": [
      {
        "type": "NON_EMPTY"
      },
      {
        "type": "NON_NEGATIVE_NUMBER"
      }
    ],
    "help": "Leave the default value (1) if no additional information was given from the Qwarry AdOps team"
  },
  {
    "type": "SELECT",
    "name": "postclickAttributionDuration",
    "displayName": "Conversion postclick attribution",
    "macrosInSelect": false,
    "selectItems": [
      {
        "value": 24,
        "displayValue": "24 hours"
      },
      {
        "value": 48,
        "displayValue": "48 hours"
      },
      {
        "value": 168,
        "displayValue": "7 days"
      }
    ],
    "simpleValueType": true,
    "help": "It represents the duration under which the conversion is attributed to Qwarry",
    "valueValidators": [
      {
        "type": "NON_EMPTY"
      }
    ],
    "enablingConditions": [
      {
        "paramName": "tagType",
        "paramValue": "conversion",
        "type": "EQUALS"
      }
    ],
    "defaultValue": 24
  }
]


___SANDBOXED_JS_FOR_WEB_TEMPLATE___

const localStorage = require("localStorage");
const getQueryParameters = require("getQueryParameters");
const getTimestampMillis = require("getTimestampMillis");
const encodeUriComponent = require("encodeUriComponent");
const sendPixel = require("sendPixel");
const injectHiddenIframe = require("injectHiddenIframe");

const QWARRY_ID_URL_QUERY_PARAM_NAME = "qwarryid";
const QWARRY_ID_STORAGE_KEY_NAME = "qwarry_id";
const QWARRY_ID_CREATED_AT_STORAGE_KEY_NAME = "qwarry_id_created_at";

// URL query param always takes precedence over the storage
let qwarryId = getQueryParameters(QWARRY_ID_URL_QUERY_PARAM_NAME) || "";

if (data.tagType === "conversion" && !qwarryId) {
  qwarryId = localStorage.getItem(QWARRY_ID_STORAGE_KEY_NAME) || "";
  const createdAt = localStorage.getItem(QWARRY_ID_CREATED_AT_STORAGE_KEY_NAME);

  // Qwarry ID gets destroyed from the storage once consumed
  if (qwarryId !== "") {
    localStorage.removeItem(QWARRY_ID_STORAGE_KEY_NAME);
    localStorage.removeItem(QWARRY_ID_CREATED_AT_STORAGE_KEY_NAME);
  }

  if (createdAt) {
    const diff = (getTimestampMillis() - createdAt); // ms

    // If over the configured postclick attribution duration, Qwarry ID is not forwarded
    if (diff >= (data.postclickAttributionDuration * 3600000)) {
      qwarryId = "";
    }
  }
}

// Postback is always done, even with an empty qwarryId
const qwarryPostbackUrl = "https://event-logger.qwarry.co/event/conversion?id="+ encodeUriComponent(data.advertiserId)+
      "&trackingId="+ encodeUriComponent(qwarryId)+
      "&value="+ encodeUriComponent(data.customValue);

if (data.postbackMethod === "iframe") {
  injectHiddenIframe(qwarryPostbackUrl, data.gtmOnSuccess);
} else {
  sendPixel(qwarryPostbackUrl, data.gtmOnSuccess);
}

data.gtmOnSuccess();


___WEB_PERMISSIONS___

[
  {
    "instance": {
      "key": {
        "publicId": "access_local_storage",
        "versionId": "1"
      },
      "param": [
        {
          "key": "keys",
          "value": {
            "type": 2,
            "listItem": [
              {
                "type": 3,
                "mapKey": [
                  {
                    "type": 1,
                    "string": "key"
                  },
                  {
                    "type": 1,
                    "string": "read"
                  },
                  {
                    "type": 1,
                    "string": "write"
                  }
                ],
                "mapValue": [
                  {
                    "type": 1,
                    "string": "qwarry_id"
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": true
                  }
                ]
              },
              {
                "type": 3,
                "mapKey": [
                  {
                    "type": 1,
                    "string": "key"
                  },
                  {
                    "type": 1,
                    "string": "read"
                  },
                  {
                    "type": 1,
                    "string": "write"
                  }
                ],
                "mapValue": [
                  {
                    "type": 1,
                    "string": "qwarry_id_created_at"
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": true
                  }
                ]
              }
            ]
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  },
  {
    "instance": {
      "key": {
        "publicId": "send_pixel",
        "versionId": "1"
      },
      "param": [
        {
          "key": "allowedUrls",
          "value": {
            "type": 1,
            "string": "specific"
          }
        },
        {
          "key": "urls",
          "value": {
            "type": 2,
            "listItem": [
              {
                "type": 1,
                "string": "https://event-logger.qwarry.co/event/conversion*"
              }
            ]
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  },
  {
    "instance": {
      "key": {
        "publicId": "inject_hidden_iframe",
        "versionId": "1"
      },
      "param": [
        {
          "key": "urls",
          "value": {
            "type": 2,
            "listItem": [
              {
                "type": 1,
                "string": "https://event-logger.qwarry.co/event/conversion*"
              }
            ]
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  },
  {
    "instance": {
      "key": {
        "publicId": "get_url",
        "versionId": "1"
      },
      "param": [
        {
          "key": "urlParts",
          "value": {
            "type": 1,
            "string": "any"
          }
        },
        {
          "key": "queriesAllowed",
          "value": {
            "type": 1,
            "string": "any"
          }
        }
      ]
    },
    "isRequired": true
  }
]


___TESTS___

scenarios: []
setup: ''


___NOTES___

Created on 08/09/2020 à 17:53:47


